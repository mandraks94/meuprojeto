...
// ==/UserScript==
        ...
        (function() {
                ...
                if (window.location.href.includes("instagram.com")) {
                        function injectMenu() {
                            ...
                            menu.innerHTML = `
                                ...
                                <div class="menu-item">
                                    <button id="inactiveAccountsBtn">😴</button>
            ...
            document.getElementById("inactiveAccountsBtn").addEventListener("click", () => {
                closeMenu();
                iniciarProcessoContasInativas();
            });
            ...
            document.getElementById("setting// ==UserScript==
// @name         Instagram Tools_2
// @description  Adds download buttons to Instagram stories
// @author       You
// @version      1.0
            ...
            function simulateClick(element, triggerChangeEvent = false) {
                 ...
                 if (triggerChangeEvent) {
                     dispatch(new Event('change', { bubbles: true }));
                            ...
                            // --- LÓGICA PARA "CONTAS INATIVAS" ---
                            async function iniciarProcessoContasInativas() {
                                if (document.getElementById("inactiveAccountsModal")) return;
                                ...
                                const fetchUserListAPISeguindo = async (userId, type, total) => {
                                    ...
                                    const setupProgressBar = () => {
                                        ...
                                        bar.style.cssText = "position:fixed;top:20px;left:50%;transform:translateX(-50%);width:80%;height:30px;background:#ccc;z-index:10001;color:black;font-weight:bold;font-size:14px;text-align:center;line-height:30px;";
                                        fill = document.createElement("div"); fill.style.cssText = "height:100%;width:0%;background:#4caf50;position:absolute;left:0;top:0;z-index:-1;";
                                        text = document.createElement("div"); text.style.position = "relative";
                                        bar.appendChild(fill); bar.appendChild(text); document.body.appendChild(bar);
                                    };
                                ...
                                try {
                                    update(0, 1, "Buscando sua lista de 'seguindo'...");
                                    const profileInfoResponse = await fetch(`https://www.instagram.com/api/v1/users/web_profile_info/?username=${username}`, { headers: { 'X-IG-App-ID': appID } });
                                    const profileInfo = await profileInfoResponse.json();
                                    const userId = profileInfo.data?.user?.id;
                                    ...
                                    const totalFollowing = profileInfo.data.user.edge_follow.count;
                                    const seguindoList = await fetchUserListAPISeguindo(userId, 'following', totalFollowing);

                                    if (processoCancelado) return;

                                    const inactiveUsers = [];
                                    const settings = loadSettings();
                                    const batchSize = settings.analysisBatchSize || 1; // Usa o valor das configurações, padrão 1
                                    const baseDelay = settings.analysisDelay || 3000; // Usa o valor das configurações, padrão 3000ms
                                    
                                    for (let i = 0; i < seguindoList.length; i += batchSize) {
                                        if (processoCancelado) break;

                                        const batch = seguindoList.slice(i, i + batchSize);
                                        const promises = batch.map(async (user) => {
                                            try {
                                                const userInfoResponse = await fetch(`https://www.instagram.com/api/v1/users/web_profile_info/?username=${user.username}`, { headers: { 'X-IG-App-ID': appID } });
                                                const userInfo = await userInfoResponse.json();
                                                const userData = userInfo.data.user;

                                                const reasons = [];
                                                // Critério 1: Seguindo > Seguidores
                                                if (userData.edge_follow.count > userData.edge_followed_by.count) {
                                                    reasons.push('Segue mais do que é seguido');
                                                }

                                                // Critério 2: Sem foto de perfil
                                                if (userData.is_private && userData.has_anonymous_profile_picture) {
                                                    reasons.push('Sem foto de perfil');
                                                } else if (!userData.is_private && userData.profile_pic_url.includes('s150x150')) {
                                                    const defaultPicIdentifier = "44884218_345707102882519_2446069589734326272_n.jpg";
                                                    if(userData.profile_pic_url.includes(defaultPicIdentifier)){
                                                        reasons.push('Sem foto de perfil');
                                                    }
                                                }
                                                ...
                                                console.warn(`Falha ao analisar ${user.username}:`, err);
                                            }
                                            return null;
                                        });

                                        const results = await Promise.all(promises);
                                        results.filter(Boolean).forEach(user => inactiveUsers.push(user));

                                        update(Math.min(i + batchSize, seguindoList.length), seguindoList.length, `Analisando perfis:`);

                                        // Pausa maior e randomizada para evitar bloqueio
                                        const randomDelay = Math.floor(Math.random() * 1500) + baseDelay; // Pausa com base na configuração
                                        await new Promise(r => setTimeout(r, randomDelay));
                                    }

                                    bar.remove();
                                    if (!processoCancelado) {
                                        abrirModalContasInativas(inactiveUsers);
                                    }
                            ...
                            function abrirModalContasInativas(users) {
                                if (document.getElementById("inactiveAccountsModal")) return;

                                const div = document.createElement("div");
                                div.id = "inactiveAccountsModal";
                                div.className = "submenu-modal";
                                div.style.cssText = `
                                    position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
                                    width: 90%; max-width: 800px; max-height: 90vh; border: 1px solid #ccc;
                                    border-radius: 10px; z-index: 10000; overflow: auto;
                                `;
                                ...
                                const renderPage = (page) => {
                                    ...
                                    let tableHtml = `
                                        <table style="width: 100%; border-collapse: collapse;">
                                            <thead>
                                                <tr style="text-align: left; border-bottom: 2px solid #dbdbdb;">
                                                    <th style="padding: 8px;">Usuário (${users.length})</th>
                                                    <th style="padding: 8px;">Motivos</th>
                                                </tr>
                                            </thead>
                                            <tbody>`;

                                    paginatedUsers.forEach(({ username, photoUrl, reasons }) => {
                                        tableHtml += `
                                            <tr style="border-bottom: 1px solid #dbdbdb;">
                                                <td style="padding: 8px; display:flex; align-items:center; gap:10px;">
                                                    <img src="${photoUrl}" alt="${username}" style="width:40px; height:40px; border-radius:50%;">
                                                ...
                                                <td style="padding: 8px;">
                                                    ${reasons.map(r => `<span style="background: #e74c3c; color: white; padding: 2px 6px; border-radius: 4px; font-size: 12px; margin-right: 5px;">${r}</span>`).join('')}
                                                </td>
                                            </tr>`;
                                    });
                                    tableHtml += `</tbody></table>`;
                                    ...
                                    const totalPages = Math.ceil(users.length / itemsPerPage);
                                    let paginationHtml = `<div style="display:flex; justify-content:center; align-items:center; gap:10px; margin-top:20px;">`;
                                    if (page > 1) paginationHtml += `<button id="prevPageBtnInactive" style="padding:5px 10px;">Anterior</button>`;
                                    paginationHtml += `<span>Página ${page} de ${totalPages}</span>`;
                                    if (page < totalPages) paginationHtml += `<button id="nextPageBtnInactive" style="padding:5px 10px;">Próximo</button>`;
                                    paginationHtml += `</div>`;

                                    div.innerHTML = `
                                        <div class="modal-header">
                                            <span class="modal-title">Contas Inativas Encontradas</span>
                                            <div class="modal-controls"><button id="fecharInactiveBtn" title="Fechar">X</button></div>
                                        </div>
                                        <div style="padding: 15px;">${tableHtml}${paginationHtml}</div>
                                    `;

                                    document.body.appendChild(div);

                                    document.getElementById("fecharInactiveBtn").onclick = () => div.remove();
                                    document.getElementById("prevPageBtnInactive")?.addEventListener('click', () => renderPage(--currentPage));
                                    document.getElementById("nextPageBtnInactive")?.addEventListener('click', () => renderPage(++currentPage));
                                };
                            ...
                            function loadSettings() {
                                const defaults = {
                                    ...
                                    itemsPerPage: 10,
                                    language: 'pt-BR',
                                    analysisBatchSize: 1, // Mais seguro: processa 1 perfil por vez
                                    analysisDelay: 3000   // Mais seguro: 3 segundos de atraso base
                                };
                                try {
                            ...
                            function abrirModalParametros() {
                                ...
                                div.innerHTML = `
                                    ...
                                    <div style="padding: 20px; display: flex; flex-direction: column; gap: 20px;">
                                        ...
                                        <div style="display: flex; justify-content: space-between; align-items: center;">
                                            <label for="analysisBatchSizeInput" title="Número de perfis para analisar de uma vez. Menor = mais seguro.">Tamanho do Lote de Análise</label>
                                            <input type="number" id="analysisBatchSizeInput" value="${settings.analysisBatchSize}" style="width: 80px;">
                                        </div>
                                        <div style="display: flex; justify-content: space-between; align-items: center;">
                                            <label for="analysisDelayInput" title="Atraso base entre os lotes (em ms). Maior = mais seguro.">Atraso entre Lotes (ms)</label>
                                            <input type="number" id="analysisDelayInput" value="${settings.analysisDelay}" style="width: 80px;">
                                        </div>
                                        <div style="display: flex; justify-content: space-between; align-items: center;">
                                ...
                                document.getElementById("saveParamsBtn").onclick = () => {
                                    const newSettings = {
                                        ...
                                        analysisBatchSize: parseInt(document.getElementById("analysisBatchSizeInput").value, 10),
                                        analysisDelay: parseInt(document.getElementById("analysisDelayInput").value, 10),
                        ...
                        async function performActionOnProfile(users, menuTexts, callback) {
                            ...
                            function unfollowSelecionados(e, tableId = "naoSegueDeVoltaTable") {
                                ...
                                // ... resto da função unfollowSelecionados (a lógica original está abaixo)
                            }
