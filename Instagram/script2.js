        ...
        function renderPage(page) {
            ...
            if (currentTab === 'selecionados') {
                filteredUsers = users.filter(({ username }) => {
                    return officialCheckboxStates.get(username) === true;
                });
            ...
            pageUsers.forEach(({ username, photoUrl }, idx) => {
                ...
                if (currentTab === 'nao_selecionados') {
                    isChecked = false; // No users selected by default in "nao_selecionados" tab
                } else {
                    isChecked = true; // For selecionados, all are checked
                }

                html += `
                    <li style="padding:5px 0;border-bottom:1px solid #eee;display:flex;align-items:center;gap:10px;">
                        <label class="custom-checkbox" for="cfcb_${globalIdx}" style="margin:0;">
                            <input type="checkbox" class="closeFriendCheckbox" id="cfcb_${globalIdx}" data-username="${username}" ${isChecked ? "checked" : ""}>
            ...
            document.getElementById("closeFriendsMarcarTodosBtn").onclick = () => {
                document.querySelectorAll("#closeFriendsList .closeFriendCheckbox").forEach(cb => cb.checked = true);
            };
            document.getElementById("closeFriendsDesmarcarTodosBtn").onclick = () => {
                document.querySelectorAll("#closeFriendsList .closeFriendCheckbox").forEach(cb => cb.checked = false);
            };
            ...
            document.getElementById("closeFriendsAplicarBtn").onclick = async () => {
                ...
                for (let i = 0; i < changedUsers.length; i++) {
                    ...
                    officialCheckboxStates.set(changedUsers[i].dataset.username, changedUsers[i].checked);
                    // Update modal checkboxes in real-time
                    document.querySelectorAll(".closeFriendCheckbox").forEach(cb => {
                        const username = cb.dataset.username;
                        const isChecked = officialCheckboxStates.get(username) || false;
        ...
        function renderPage(page) {
            ...
            document.getElementById("hideStoryAplicarBtn").onclick = async () => {
                ...
                for (let i = 0; i < changedUsers.length; i++) {
                    ...
                    document.querySelectorAll(".hideStoryCheckbox").forEach(cb => {
                        const username = cb.dataset.username;
                        const isChecked = officialStates.get(username) || false;
                        cb.checked = isChecked;
                    });
                }
                updateAplicarProgressBar(changedUsers.length, changedUsers.length);
